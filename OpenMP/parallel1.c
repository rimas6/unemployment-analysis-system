...
int main(int argc, char *argv[]) {
    ...
    while (fgets(buffer, 1024, fp) && total_count < MAX_RECORDS) {
        ...
        token = strtok(NULL, ",\r\n"); // Rate
        if (token != NULL) {
            data[total_count].rate = atof(token);//Reference: 
            total_count++;
        }
    }
    ...
    // --- Phase 2: Calculation ---
    printf("--- Phase 2: Processing (Parallel) ---\n");
    // int num_threads = omp_get_max_threads(); // (Optional: useful for debug)

    double urban_sum = 0.0; long urban_count = 0;
    double rural_sum = 0.0; long rural_count = 0;

    double start_time = omp_get_wtime();
    
    // استخدام التوازي مع Reduction كما هو مطلوب
    #pragma omp parallel for schedule(runtime) reduction(+:urban_sum, urban_count, rural_sum, rural_count)
    for (long i = 0; i < total_count; i++) {
        //Reference: https://www.geeksforgeeks.org/c/strcmp-in-c/
